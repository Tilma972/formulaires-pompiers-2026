// prefillform-qualification.js - Script spécifique pour le formulaire de qualification
...
(function() {
    ...
    function prefillQualificationForm() {
      try {
        ...
        // Définir la commune en lecture seule et la pré-remplir
        if (params.commune) {
          const communeField = document.getElementById('commune_entreprise');
          if (communeField) {
            // Désactiver le select et le rendre en apparence comme un champ en lecture seule
            communeField.disabled = true;
            communeField.style.backgroundColor = '#f5f5f5';
            communeField.style.border = '1px solid #ddd';
            
            // Créer et ajouter une seule option avec la commune spécifiée
            const option = document.createElement('option');
            option.value = safeDecodeURIComponent(params.commune);
            option.textContent = safeDecodeURIComponent(params.commune);
            option.selected = true;
            
            // Vider et remplir
            communeField.innerHTML = '';
            communeField.appendChild(option);
          }
        }
